...
def currentDate():
    ...
    """
    return datetime.now().strftime("%d.%m.%y")  # Return current date formatted as day.month.year.
...
def currentTime(seconds=False, microSeconds=False):
    ...
    match seconds:  # Using pattern matching to handle different cases
        case False:
            return datetime.now().strftime("%H:%M")  # Return current time formatted as hour:minute.
        case True:
            match microSeconds:  # Nested pattern matching to handle microseconds
                case True:
                    return datetime.now().strftime("%H:%M:%S.%f")  # Return current time with microseconds.
                case False:
                    return datetime.now().strftime("%H:%M:%S")  # Return current time with seconds.
...
def currentTimeStamp():
    ...
    """
    timeStamp = datetime.now().timestamp()  # Get current timestamp using datetime module.
